...
export default function TransactionPage({ txHash: txHashProp }) {
  ...
  // Update txHash when prop changes
  useEffect(() => {
    if (txHashProp) {
      setTxHash(txHashProp);
    }
  }, [txHashProp]);

  // Handle rpcUrl from URL and fetch transaction data when txHash changes
  useEffect(() => {
    ...
    if (isEthersReady && txHash) {
      fetchTransactionData();
    }
  }, [isEthersReady, txHash]);
